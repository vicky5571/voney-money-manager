...
function BalanceAdjustmentContent({
  account,
  onClose,
  onSuccess,
}: FormContentProps) {
  ...
  const difference = isValidNumber
    ? Number((parsedNewBalance - currentBalance).toFixed(2))
    : 0;
    ...
    <div className="relative w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
      ...
      {offlineCount > 0 && (
        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-2 text-amber-800 text-xs">
          ...
          <p>
            You have{" "}
            <span className="font-bold">{offlineCount} offline item(s)</span>{" "}
            pending. Sync them first to avoid overwriting recent changes.
          </p>
        </div>
      )}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="mt-1 text-[11px] text-gray-600 leading-relaxed">
                An adjustment transaction will be recorded to reconcile your
                records without penalizing your category budget limits.
              </p>
            </div>
          </div>
        )}
