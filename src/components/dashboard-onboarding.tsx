    ...
    <section className="rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4">
      ...
      <div className="mt-4 space-y-3">
        {steps.map(({ key, title, detail, href, Icon }, index) => {
          ...
          const content = (
            <>
              <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${done ? 'bg-emerald-100 text-emerald-600' : available ? 'bg-white text-indigo-600 shadow-sm' : 'bg-indigo-100 text-indigo-300'}`}>
              ...
              {!done && available && <span className="text-xs font-semibold text-indigo-600">Start</span>}
            </>
          );
          ...
          return done || !available ? (
            ...
            <Link key={key} href={href} className="flex items-center gap-3 rounded-xl outline-none transition-opacity hover:opacity-80 focus-visible:ring-2 focus-visible:ring-indigo-600">
              {content}
            </Link>
          );
        })}
