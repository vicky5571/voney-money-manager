...
export async function getBudgets(
  ...
  return budgets.map((b) => {
    const category = (
      Array.isArray(b.categories) ? b.categories[0] : b.categories
...
export async function getBudgetDetail(id: string): Promise<BudgetDetailResult> {
  ...
  const category = (
    Array.isArray(budget.categories) ? budget.categories[0] : budget.categories
  ) as BudgetCategory | null;
  const catId = category?.id ?? "";
  ...
  for (const t of transactions) {
    spentByDate[t.transaction_date] =
      (spentByDate[t.transaction_date] || 0) + t.amount;
  }

  // Find index of today in dateList
  let todayIndex = dateList.indexOf(todayStr);
  if (todayIndex === -1) {
    if (todayStr < dateList[0])
      todayIndex = -1; // Before start
  ...
  const dailySafeSpend =
    daysRemaining > 0 ? Math.max(0, Math.round(remaining / daysRemaining)) : 0;
...
export async function createBudget(formData: {
  ...
  if (!parsed.success)
    throw new Error(parsed.error.issues[0]?.message || "Invalid budget input");
  ...
  if (
    catRow.user_id !== null &&
    catRow.user_id !== user.id &&
    !catRow.is_default
  )
    throw new Error("Category access denied");
